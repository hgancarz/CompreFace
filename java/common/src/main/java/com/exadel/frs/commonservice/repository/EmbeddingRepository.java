...
public interface EmbeddingRepository extends JpaRepository<Embedding, UUID> {
    ...
    @Query("select new com.exadel.frs.commonservice.projection.EnhancedEmbeddingProjection(e.id, e.embedding, s.subjectName) " +
           "from Embedding e " +
    ...
    @Query("select new com.exadel.frs.commonservice.projection.EmbeddingProjection(e.id, e.subject.subjectName) " +
           "from Embedding e " +
