...
public interface EmbeddingRepository extends JpaRepository<Embedding, UUID> {
    ...
    @Query("select new com.exadel.frs.commonservice.projection.EnhancedEmbeddingProjection(e.id, e.embedding, s.subjectName) from Embedding e left join e.subject s where s.apiKey = :apiKey")
    ...
    @Query("select new com.exadel.frs.commonservice.projection.EmbeddingProjection(e.id, e.subject.subjectName) from Embedding e where e.subject.apiKey = :apiKey")
    Page<EmbeddingProjection> findBySubjectApiKey(String apiKey, Pageable pageable);

    @Query("select new com.exadel.frs.commonservice.projection.EmbeddingProjection(e.id, e.subject.subjectName) from Embedding e where e.subject.apiKey = :apiKey and (cast(:subjectName as string) is null or e.subject.subjectName = :subjectName)")
    ...
    @Query("select count(e) from Embedding e where e.subject.apiKey = :apiKey and e.calculator <> :calculator")
