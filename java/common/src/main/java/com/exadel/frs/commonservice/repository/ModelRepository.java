...
public interface ModelRepository extends JpaRepository<Model, Long> {
    ...
    @Query("select new com.exadel.frs.commonservice.projection.ModelSubjectProjection(m.guid, count(s.id)) from Model m left join Subject s on m.apiKey = s.apiKey group by m.guid")
    List<ModelSubjectProjection> getModelSubjectsCount();

    @Query("select distinct new com.exadel.frs.commonservice.projection.ModelProjection(m.guid, m.name, m.apiKey, m.type, m.createdDate) from Model m left join m.app a where a.id = :appId")
